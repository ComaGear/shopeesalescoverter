...
public class TempMovementReporting {
    ...
    private static void writeBiztoryMoveOutSheet(XSSFSheet biztorySheet, List<MoveOut> moveOuts){
        ...
        for(MoveOut moveOut : moveOuts){
            ...
            row.createCell(4).setCellValue(MoveOutFactory.getSKUSubTotal(moveOut) / moveOut.getQuantity());
        }
    }
        ...
        for(MoveOut moveOut : moveOuts){
            ...
            if(moveOut.getProductId() != null) {
                ...
                uom = new UOM();
                uom.setProductId("");
                uom.setCostPrice(moveOut.getFinalPrice());
            }
        ...
        for(MoveOut moveOut : moveOuts){
            ...
            if(lastOrder.getId() != null && lastOrder.getId().equals(moveOut.getOrder().getId())) {
               ...
               lastOrder.setTotalAmount(lastOrder.getTotalAmount() + MoveOutFactory.getSKUSubTotal(moveOut));
                ...
                lastOrder.setStatus(MoveOutFactory.getPlatformOrderStatus(moveOut));
